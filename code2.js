...
var static = {championId: [[],[],[],[],[],[],[],[],[],[],[]], championName: [[],[],[],[],[],[],[],[],[],[],[]], item: '0'};
var summoner = new Object();
...
$(document).ready(function(){    
    ...
    $('.send').click(function(event){
        data.name = noAcentos($('.name').val().replace(/ /g,''));
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            jQuery.each(response.games, function(game){
                                ...
                                static.championId[0][game] = response.games[game].championId;
                                ...
                                success: function(response) {
                                    for (game=0;game<10;game++){
                                        for (player=0;player<9;player++){
                                            static.championId[game][player] = response.games[game].fellowPlayers[player].championId;
                            ...
                            $.ajax({
                                url: 'https://' + data.server + '.api.pvp.net/api/lol/static-data/' + data.server + '/v1.2/champion?dataById=true&api_key=a0797630-9996-4ab6-85d6-704029984adf',
                                //https://na.api.pvp.net/api/lol/static-data/' + data.server + '/v1.2/champion/' + static.championId + '?api_key=a0797630-9996-4ab6-85d6-704029984adf',
                                dataType: 'json',
                                success: function(response) {
                                    for (game = 1; game < 11; game++){
                                        //static.championName[0][game] = response.data[static.championId[0][game]].key;
//                                        for (player = 0; player < 9; player++){
//                                            console.log(static.championId[game][player]);
//                                            static.championName[game][player] = response.data[static.championId[game-1][player]].key;
//                                            $('.players:eq(' + game + '.outChampTeam:eq(' + player + ')').attr('src', 'http://ddragon.leagueoflegends.com/cdn/' + data.ver + '/img/champion/' + static.championName[game][player] + '.png');
//                                        };
                                        //console.log(static.championName);
                                        $('.outChamp:eq(' + game + ')').attr('src', 'http://ddragon.leagueoflegends.com/cdn/' + data.ver + '/img/champion/' + static.championName[0][game] + '.png');
                                        $('.outChampionName:eq(' + game + ')').text(static.championName[0][game]);
                                    }
                                    $('.Result').show(100);
                                }
                            });
